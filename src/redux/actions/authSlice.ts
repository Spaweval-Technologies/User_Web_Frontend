...
export const signupUser = createAsyncThunk(
  ...
  async (payload: SignupPayload, { rejectWithValue }) => {
    try {
      const res = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/api/auth/signup`, payload, {
      ...
      return (
        rejectWithValue(err.response?.data) || rejectWithValue('Signup failed: ' + err.message)
      );
    }
  }
);
  ...
  async (payload: { token: string }, { rejectWithValue }) => {
    try {
      const res = await axios.get(
        `${process.env.NEXT_PUBLIC_API_URL}/api/auth/signup/verify-token`,
        {
          headers: {
      ...
      return (
        ...
        rejectWithValue('token authentication error' + err.message)
      );
    }
  }
);
...
export const verifySignupOtp = createAsyncThunk(
  ...
  async ({ email, otp }: { email: string; otp: string }, { rejectWithValue }) => {
    try {
      ...
      return (
        rejectWithValue(err?.response?.data) ||
        rejectWithValue(err.message || 'An error occurred while verifying OTP')
      );
    }
  }
);
...
export const logout = createAsyncThunk(
  'auth/logout',
  async ({ email, auth_token }: { email: string; auth_token: string }, { rejectWithValue }) => {
    try {
      const res = await axios.post(
      ...
      if (res.status !== 200) {
        return rejectWithValue(res.data || 'Logout failed');
      }
      return res.data;
    } catch (err: any) {
